...
test_succeeds_windows('download URLs_IMDB Datablock', {
  path = 'imdb'
  bs = 20
...
test_succeeds_windows('download URLs_IMDB Datablock dataloaders', {
  dbunch_lm = imdb_lm %>% dataloaders(source = path, path = path, bs = bs, seq_len = 10,num_workers=0L)
})

test_succeeds_windows('download URLs_IMDB Datablock leaner', {
